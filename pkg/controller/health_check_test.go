...
func (s *TestHealthCheckSuite) TestHealthCheckHandler() {
	...
	healthCheckCtrl := controller.NewHealthCheck(controller.NewHealthChecker(proxy.DefaultProxyPort))
	...
	s.Run("health in testing mode", func() {
		...
		// mock proxy
		defer gock.Off()
		gock.New(fmt.Sprintf("http://localhost:%s", proxy.DefaultProxyPort)).
			Get("/proxyhealth").
			Persist().
			Reply(http.StatusOK).
			BodyString("")
