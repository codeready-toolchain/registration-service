...
func (ns *TestNamespacesSuite) TestResetNamespacesHandler() {
	...
	ns.Run(`handler returns a "Not Found" error when the user signup cannot be found or is deactivated`, func() {
		...
		test.AssertError(ns.T(), rr, http.StatusNotFound, ErrNamespaceReset.Error(), "The user is either not found or deactivated. Please contact the Developer Sandbox team at devsandbox@redhat.com for assistance")
	})
	...
	ns.Run(`handler returns a "Bad Request" error when the user does not have any provisioned namespaces`, func() {
		...
		test.AssertError(ns.T(), rr, http.StatusBadRequest, ErrNamespaceReset.Error(), "No namespaces provisioned, unable to perform reset. Please try again in a while and if the issue persists, please contact the Developer Sandbox team at devsandbox@redhat.com for assistance")
	})
	...
	ns.Run(`handler returns an internal server error when a generic error occurs`, func() {
		...
		// returned.
		test.AssertError(ns.T(), rr, http.StatusInternalServerError, ErrNamespaceReset.Error(), "Unable to reset your namespaces. Please try again in a while and if the issue persists, please contact the Developer Sandbox team at devsandbox@redhat.com for assistance")
	})
}
