...
func (s *ServiceImpl) Signup(ctx *gin.Context) (*v1alpha1.UserSignup, error) {
	...
	username := ctx.GetString(context.UsernameKey)
	return nil, errors.NewConflict(schema.GroupResource{}, "", fmt.Errorf("unable to create UserSignup [id: %s; username: %s] because there is already an active UserSignup with such ID", encodedUserID, username))
}
