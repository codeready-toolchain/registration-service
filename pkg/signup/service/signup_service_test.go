...
func (s *TestSignupServiceSuite) TestFailsIfUserSignupNameAlreadyExists() {
	...
	require.EqualError(s.T(), err, fmt.Sprintf("Operation cannot be fulfilled on  \"\": unable to create UserSignup [id: %s; username: jsmith] because there is already an active UserSignup with such ID", userID.String()))
}
