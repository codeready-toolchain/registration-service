...
func (s *ServiceImpl) performPhoneLookup(ctx *gin.Context, cfg configuration.RegistrationServiceConfig,
	...
	if lookupErr != nil {
		PhoneLookupErrorsTotal.WithLabelValues(sender.LookupErrorType(lookupErr), noProvisioningLabel(signup)).Inc()
	...
	if isHighRiskPhone(result) {
		...
		if mode == toolchainv1alpha1.PhoneLookupModeEnabled {
			...
			PhoneLookupTotal.WithLabelValues(PhoneLookupResultBlocked, result.CarrierRiskCategory, noProvisioningLabel(signup)).Inc()
		...
		PhoneLookupTotal.WithLabelValues(PhoneLookupResultAllowed, result.CarrierRiskCategory, noProvisioningLabel(signup)).Inc()
		return false, nil
	}
	PhoneLookupTotal.WithLabelValues(PhoneLookupResultAllowed, result.CarrierRiskCategory, noProvisioningLabel(signup)).Inc()
	return false, nil
